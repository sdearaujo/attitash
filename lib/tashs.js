...
exports.addTash = function(username, content, callback){
	db.run("insert into Tashs values (?, ?, datetime('now', 'localtime'))",
		username, content,
		exports.getLastTash(username, callback));
};

exports.getTashsByHashTag = function(hashTag, callback){
        db.all('select u.fname, u.lname, u.uname, t.content, t.tdate from Users u, Tashs t where t.content like ? and t.uname=u.uname order by t.tdate desc',
                hashTag,
                callback);
};

exports.getTashsByMention = function(uname, callback){
        db.all('select u.fname, u.lname, u.uname, t.content, t.tdate from Users u, Tashs t where t.content like ? and t.uname=u.uname order by t.tdate desc',
                uname,
                callback);
};
